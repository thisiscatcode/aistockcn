...
import {
  getPaperDbDailyHistory,
  getPaperHoldings,
  getPaperTargets,
  getPortfolioOverview,
  type OverviewTopPick
} from "@/lib/api";
...
function formatSignedNumber(value: number | null) {
  ...
  const formatted = new Intl.NumberFormat("en-US", { maximumFractionDigits: 0 }).format(Math.abs(Math.round(value)));
  return `${value > 0 ? "+" : value < 0 ? "-" : ""}${formatted}`;
}

function formatIntegerDisplay(value: unknown, locale: PanelLocale) {
  const numericValue = asNumber(value);
  if (numericValue === null) return formatDisplayValue(value, { locale });
...
function calendarCells(dailyRows: DashboardRow[], selectedMonthDate?: Date | null) {
  ...
  const cells: Array<{ key: string; day: number; inMonth: boolean; pnl: number | null; trades: number }> = [];
  ...
  return { monthDate: monthStart, cells };
}

function PnlCalendar({ dailyRows, monthDate, locale }: { dailyRows: DashboardRow[]; monthDate?: Date | null; locale: PanelLocale }) {
  ...
  return (
    <section className="panel overview-calendar-panel" aria-label="Daily trading P&amp;L calendar">
      <div className="overview-calendar-toolbar">
        <a className="calendar-nav-button" href={`/cn/quant?month=${monthKey(previousMonth)}`} aria-label={`Previous month, ${monthTitle(previousMonth, locale)}`}>&lt;</a>
        <span className="pill">{monthTitle(pnlCalendar.monthDate, locale)}</span>
        <a className="calendar-nav-button" href={`/cn/quant?month=${monthKey(nextMonth)}`} aria-label={`Next month, ${monthTitle(nextMonth, locale)}`}>&gt;</a>
      </div>
      <div className="pnl-calendar">
        <div className="pnl-calendar-weekdays">
          {["Mon", "Tue", "Wed", "Thu", "Fri"].map((label) => <span key={label}>{label}</span>)}
        </div>
        <div className="pnl-calendar-grid">
          {pnlCalendar.cells.map((cell) => {
            ...
            const body = <>
              ...
              {cell.inMonth ? cell.trades ? <><strong>{cell.pnl === null ? "—" : formatSignedNumber(cell.pnl)}</strong><span>{`${formatNumber(cell.trades, locale)} trades`}</span></> : <strong className="pnl-calendar-empty">——</strong> : null}
            </>;
            return cell.inMonth && cell.trades ? (
              <a className={`${className} pnl-calendar-day-link`} href={`/holdings/daily/${encodeURIComponent(cell.key)}`} key={cell.key} rel="noopener noreferrer" target="_blank">{body}</a>
            ) : <div className={className} key={cell.key}>{body}</div>;
          })}
        </div>
      </div>
    </section>
...
function KpiCard({ label, value, detail, tone = "neutral" }: { label: string; value: string; detail?: string; tone?: "positive" | "negative" | "neutral" }) {
  return <article className={`overview-kpi-card overview-kpi-${tone}`}>
...
function signalTone(signalType?: string | null) {
  ...
  if (normalized === "SELL") return "signal-short";
  return "";
}
...
function plannedOrderSide(row: DashboardRow) {
  ...
  if (buyQty !== null && buyQty > 0) return "BUY";
  if (sellQty !== null && sellQty > 0) return "SELL";
  if (deltaQty !== null && deltaQty > 0) return "BUY";
  if (deltaQty !== null && deltaQty < 0) return "SELL";
...
export async function CnQuantPerformancePage({ month }: { month?: string }) {
  const user = await requireAuth();
  const requestedMonthDate = parseMonthParam(month);
    ...
    <Shell title="CN Quant Performance" subtitle="Daily paper-trading results, positions and planned orders" locale={user.locale} username={user.username} role={user.role} market="CN" tone="light">
      ...
      <section className="overview-summary-layout" aria-label="Portfolio summary">
        ...
        <div className="overview-kpi-grid overview-kpi-stack" aria-label="Portfolio key performance indicators">
          <KpiCard label="Account Equity" value={formatIntegerDisplay(overview.account.total_assets, user.locale)} detail={`Updated ${formatDateTime(overview.account.updated_at ?? overview.generated_at, user.locale)}`} />
          <KpiCard label="Today P&L" value={formatIntegerDisplay(overview.account.today_pnl, user.locale)} detail={formatPercent(overview.account.today_pnl_pct, user.locale)} tone={toneFromNumber(overview.account.today_pnl)} />
          <KpiCard label="Holdings / Pending Buy / Pending Sell" value={`${formatNumber(overview.positions.holding_count, user.locale)} / ${formatNumber(overview.positions.pending_buy_count, user.locale)} / ${formatNumber(overview.positions.pending_sell_count, user.locale)}`} detail={`${formatNumber(overview.positions.open_order_count, user.locale)} open orders`} />
          <KpiCard label="Pending Actions" value={formatNumber(overview.signals.pending_actions, user.locale)} detail={`Signal date ${formatDate(overview.signals.latest_signal_date, user.locale)}`} />
        </div>
      </section>
      ...
      <section className="portfolio-table-card">
        <div className="portfolio-card-header">
          <div><p className="portfolio-section-kicker">Actionable Insights</p><h2>Top AI Picks</h2></div>
          <span className="portfolio-table-aside">Source: {overview.top_picks[0]?.source ?? "—"}</span>
        </div>
        <div className="portfolio-table-wrap">
          <table className="portfolio-picks-table">
            <thead><tr><th>Ticker/Symbol</th><th>Company Name</th><th>Signal Type</th><th>AI Confidence Score</th><th>Recommended Weight</th></tr></thead>
            <tbody>{overview.top_picks.map((pick: OverviewTopPick, index) => {
              const code = normalizeSymbol(pick.code);
              return <tr key={`${pick.code ?? "row"}-${index}`}>
                <td>{code ? <a className="data-table-link" href={paperStockUrl(code)}><strong>{code}</strong></a> : <strong>—</strong>}</td>
                <td>{pick.name ?? "—"}</td>
                <td><span className={`signal-badge ${signalTone(pick.signal_type)}`}>{pick.signal_type ?? "—"}</span></td>
                <td><div className="confidence-cell"><div className="confidence-track" aria-hidden="true"><span style={{ width: `${scoreWidth(pick.confidence)}%` }} /></div><strong>{formatScore(pick.confidence, user.locale)}</strong></div></td>
      ...
      <Panel title="Planned Orders" aside={<span className="pill">{formatNumber(intendedOrderRows.length, user.locale)} intended</span>}>
        <p className="table-note">Intended buy/sell orders from the latest rebalance target file. These remain visible before broker submission and outside active trading hours.</p>
