    ...
    <Shell title="Quant" subtitle="Signals, validation and market data" locale={user.locale} username={user.displayName} role={user.role} market={market}>
      ...
      {view === "methodology" ? (
        <div className="case-capability-grid quant-method-grid">
          <Panel title="Market isolation"><p className="panel-copy">{market === "US" ? "US calendar, adjusted prices, USD costs and us_5d_v1 artifacts are isolated from CN stock production." : "China calendar, price limits, lot size, fees and model artifacts remain market-specific."}</p></Panel>
