...
class FeiKeywordUsError(RuntimeError):
    pass
...
select
  ...
  k.created_at,
  k.updated_at,
  count(m.symbol) as mapped_stock_count
from us_stock_keywords k
left join us_stock_key_map m
...
def _connect(settings: Settings):
    if not settings.paper_db_url:
        raise FeiKeywordUsError("PAPER_DB_URL is not configured")
    if psycopg is None or dict_row is None:
        raise FeiKeywordUsError("psycopg is not installed")
...
def normalize_keyword_ids(raw_ids: Any) -> list[int]:
    if not isinstance(raw_ids, list):
        raise FeiKeywordUsError("invalid_keyword_ids")
    ...
    for raw_id in raw_ids:
        if isinstance(raw_id, bool) or not isinstance(raw_id, int) or raw_id < 1:
            raise FeiKeywordUsError("invalid_keyword_ids")
        if raw_id in seen:
            raise FeiKeywordUsError("duplicate_keyword_id")
        seen.add(raw_id)
        keyword_ids.append(raw_id)
    return keyword_ids


def list_us_keywords(*, favorites_only: bool = False) -> dict[str, Any]:
...
def replace_us_favorite_keywords(raw_keyword_ids: Any) -> dict[str, Any]:
    keyword_ids = normalize_keyword_ids(raw_keyword_ids)
    settings = get_settings()
    with _connect(settings) as conn:
        try:
            with conn.cursor() as cur:
                cur.execute("select id from us_stock_keywords order by id for update")
                existing_ids = {int(row["id"]) for row in cur.fetchall()}
                if any(keyword_id not in existing_ids for keyword_id in keyword_ids):
                    raise FeiKeywordUsError("keyword_not_found")
                cur.execute(
                    """
                    update us_stock_keywords
                    set fav_flg = false,
                        display_num = null,
                        updated_at = now()
                    where fav_flg = true
